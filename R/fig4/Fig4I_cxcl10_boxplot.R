# ============================================================
# Figure:  Figure 4I
# Title:   Cxcl10 expression boxplot — ChAd vs PBS, Days 1, 2 & 3
...
# Input:   Fig4I_cxcl10_boxplot.xlsx  (sheet: biomarker_raw)
# Output:  Fig4I_cxcl10_boxplot.pdf   (boxplot)
# ============================================================
...
if (nrow(cxcl10) > 1) {
  ...
  means <- rowMeans(sapply(cxcl10[, fpkm_cols_all], as.numeric), na.rm = TRUE)
  cxcl10 <- cxcl10[which.max(means), ]
}

cat("Found Cxcl10 row — gene ID:", cxcl10$ID[1], "\n")
...
long_list <- lapply(fpkm_map, function(entry) {
  vals <- suppressWarnings(as.numeric(cxcl10[, entry$cols]))
  data.frame(
    day       = entry$day,
...
pval_df <- data.frame(
  ...
  padj  = sapply(pval_cols, function(col) {
    suppressWarnings(as.numeric(cxcl10[[col]]))
  }),
  stringsAsFactors = FALSE
)
...
ggsave("Fig4I_cxcl10_boxplot.pdf", plot = p, width = 9, height = 7, device = cairo_pdf)
save_source_data(p, "Fig4I_cxcl10_boxplot.pdf")
cat("Plot saved to Fig4I_cxcl10_boxplot.pdf\n")
